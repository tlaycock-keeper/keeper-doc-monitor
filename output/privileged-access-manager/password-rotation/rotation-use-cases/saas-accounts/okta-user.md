...
[PreviousCustom Scripts](/en/keeperpam/privileged-access-manager/password-
...
This documentation explains how to rotate Okta accounts using KeeperPAM's
...
Create a new PAM User record to store Okta User details whose password will be
rotated.
...
Okta SDK only supports password rotation if the current password is valid. If
the password is incorrect, the rotation will fail.
...
In order for the post-rotation script to execute, the host where the Keeper
Gateway is running needs to have the necessary execution environment
available.
