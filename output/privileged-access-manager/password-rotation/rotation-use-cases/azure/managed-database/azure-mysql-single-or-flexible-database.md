...
flexible-database?fallback=true)[Keeper Connection
...
database/azure-mysql-single-or-flexible-database?fallback=true)[User
...
In this guide, you'll learn how to rotate passwords for Azure MySQL Database
Users and Admin accounts on your Azure environment using Keeper Rotation.
Azure MySQL is an Azure managed resource where the MySQL Admin Credentials are
defined in the **PAM Database** record type and the configurations of the
MySQL Users are defined in the **PAM User** record type.

For Azure Managed MySQL database, the Azure SDK will be used to rotate the
...
The **PAM Database** record links to the admin credentials and necessary
configurations to connect to the MySQL Server on Azure. Keeper Rotation will
use these provided configurations to rotate passwords of regular database user
accounts in the Azure MySQL Server instance. These provided admin credentials
need to also have sufficient database permissions to successfully change the
...
PAM User admin account username and password that will perform rotation. If
the Admin account in the DB user table is in a Host other than %, add the Host
value to the user name as `USERNAME@HOST`
...
application and has network access to your Azure MySQL database from the pre-
requisites
