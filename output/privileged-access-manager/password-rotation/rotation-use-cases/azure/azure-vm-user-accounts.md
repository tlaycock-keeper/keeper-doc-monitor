...
rotation/rotation-use-cases/azure/azure-vm-user-accounts?fallback=true)[Keeper
...
cases/azure/azure-vm-user-accounts?fallback=true)[User
...
other machine on the network. A PAM Machine record should be created for every
machine. This PAM Machine record will be associated to a linked administrative
...
record needs to be created for each user account that will be rotated.
...
Keeper Rotation will use the credentials linked from the **PAM Machine**
record to rotate the credentials of accounts referenced by the **PAM User**
records.
