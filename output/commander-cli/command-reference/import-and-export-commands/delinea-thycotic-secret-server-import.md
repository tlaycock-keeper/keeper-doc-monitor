...
and-export-commands/delinea-thycotic-secret-server-
import?fallback=true)[Keeper Connection
...
server-import?fallback=true)[User Guides](https://docs.keeper.io/en/user-
guides/commander-cli/command-reference/import-and-export-commands/delinea-
thycotic-secret-server-import?fallback=true)[Release
...
guide/commander-cli/command-reference/import-and-export-commands/delinea-
thycotic-secret-server-import?fallback=true)[MSP
...
commands/delinea-thycotic-secret-server-import?fallback=true)[KeeperPAM and
Secrets Manager](https://docs.keeper.io/en/keeperpam/commander-cli/command-
reference/import-and-export-commands/delinea-thycotic-secret-server-
import?fallback=true)[SSO Connect On-Prem](https://docs.keeper.io/en/sso-
connect-on-prem/commander-cli/command-reference/import-and-export-
commands/delinea-thycotic-secret-server-import?fallback=true)[Keeper
...
Secret Server (Delinea/Thycotic) data into Keeper which includes private
folders, shared folders, permissions, file attachments, TOTP codes. This
process utilizes the Secret Server API to automate the process.

Note: A basic import capability is available on the Keeper Web Vault and
Desktop App which supports Thycotic XML format. Visit the vault Settings >
Import > Thycotic screen. The XML format does not include attachments or
permissions. Therefore, we recommend using the automated method as described
in this document.
...
In Secret Server admin settings, ensure Webservices are enabled
 ...
 _**Admin - > Configuration -> Edit -> Enable Webservices**_
...
`download-membership` command provides an option `--sub-folder` to control how
these folders are imported.

`--sub-folder=ignore` preserves folder structure. Folder permissions are
ignored.

`--sub-folder=flatten` folder will be moved to the root folder of the Keeper
vault as its own shared folder.
...
The TOTP codes stored in Thycotic/Delinea Secret Server can only be retrieved
by manually downloading a CSV file. The admin of Secret Server needs to go to
**Secret Server** > **Export Secrets** and select the following options:
...
Commander will attempt to build the same folder structure as Secret Server in
...
folders of other users within Secret Server. This step will only import the
...
The next time that the Admin runs the `apply-membership` command, any new
Keeper users will receive access to their Shared Folders.
