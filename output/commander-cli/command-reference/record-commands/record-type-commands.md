...
`-lf`, `--list-field <FIELD NAME>` show information about a specified field
type. Can use "*" to show all fields.

`-e`, `--example` generate example json for a record type or field. *Must use
with -lr or -lf
...
`--output <OUTPUT FILE>` output results to a given file *ignored if table
format is used
...
**Detail:** Convert legacy (General typed or untyped) records to typed
records.
...
titles. Can use "?" to match any single character and "*" to match any number
of characters
...
`-u`, `--url <URL Pattern>` Only converts records with URLs that match the
given pattern. Can use "?" to match any single character and "*" to match any
number of characters

`-n`, `--dry-run` Display the outcome of the conversion without converting any
records

`-r`, `--recursive` apply conversion to all matching records in the current
and child folders

`--ignore-ownership` convert records even if they are not owned by the current
account
