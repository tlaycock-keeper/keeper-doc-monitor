...
Azure portal -> `Azure Active Directory` -> `App Registrations` -> Select app
that was created above -> `Certificates & secrets`
...
Displayed upon Registration of a new application (under Azure portal -> `Azure
Active Directory` -> `App Registrations` -> `New Registration`.

cmdr:azure_client_id

Azure portal -> `Azure Active Directory` -> `App Registrations` -> [App name]
...
(Optional) Tells Commander to use Azure AD Key rotation. This should be either
