...
manager/policies?fallback=true)[Keeper Connection
Manager](https://docs.keeper.io/en/keeper-connection-manager/privilege-
manager/policies?fallback=true)[User Guides](https://docs.keeper.io/en/user-
guides/privilege-manager/policies?fallback=true)[Release
Notes](https://docs.keeper.io/en/release-notes/privilege-
manager/policies?fallback=true)[Enterprise
Guide](https://docs.keeper.io/en/enterprise-guide/privilege-
manager/policies?fallback=true)[MSP Guide](https://docs.keeper.io/en/msp-
guide/privilege-manager/policies?fallback=true)[SSO Connect
Cloud](https://docs.keeper.io/en/sso-connect-cloud/privilege-
manager/policies?fallback=true)[KeeperPAM and Secrets
Manager](https://docs.keeper.io/en/keeperpam/privilege-
manager/policies?fallback=true)[SSO Connect On-
Prem](https://docs.keeper.io/en/sso-connect-on-prem/privilege-
manager/policies?fallback=true)[Keeper
Bridge](https://docs.keeper.io/en/keeper-bridge/privilege-
manager/policies?fallback=true)
...
[Export as
PDF](/en/keeperpam/~gitbook/pdf?page=vDbSrpaFEyRgJGVFRYIn&only=yes&limit=100)
...
To create a new policy, click on "Create Policy" and complete the policy
details form.

**Important** : Multiple policies can be applied simultaneously to the same
device or user. When this happens, Keeper enforces all applicable policies
...
When a policy is enforced, the user must pass certain controls that are
...
If MFA is required, the user will be directed to sign up with a Keeper vault
and set up a two-factor authentication method.

###

Policy Filters

A policy affects only the users and devices which are specified in the policy
filter section. This includes the following options:
...
From the main dashboard, elevation and access events can be easily converted
into new policies or added to existing policies. Select the events and then
...
Policies are pushed and applied across the fleet of endpoints within 30
minutes.
