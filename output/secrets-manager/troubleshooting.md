...
manager/troubleshooting?fallback=true)[Keeper Connection
Manager](https://docs.keeper.io/en/keeper-connection-manager/secrets-
manager/troubleshooting?fallback=true)[User
Guides](https://docs.keeper.io/en/user-guides/secrets-
manager/troubleshooting?fallback=true)[Release
Notes](https://docs.keeper.io/en/release-notes/secrets-
manager/troubleshooting?fallback=true)[Enterprise
Guide](https://docs.keeper.io/en/enterprise-guide/secrets-
manager/troubleshooting?fallback=true)[MSP
Guide](https://docs.keeper.io/en/msp-guide/secrets-
manager/troubleshooting?fallback=true)[SSO Connect
Cloud](https://docs.keeper.io/en/sso-connect-cloud/secrets-
manager/troubleshooting?fallback=true)[KeeperPAM and Secrets
Manager](https://docs.keeper.io/en/keeperpam/secrets-
manager/troubleshooting?fallback=true)[SSO Connect On-
Prem](https://docs.keeper.io/en/sso-connect-on-prem/secrets-
manager/troubleshooting?fallback=true)[Keeper
...
In order to utilize Keeper Secrets Manager, two permission criteria must be
met:
...
When performing commands with the KSM CLI that use a record or folder UID as
an input, the command line interface cannot distinguish between a UID that
starts with "-" and a command option. If the UID you are using begins with a
"-" (dash / hyphen) simply **add "--" before the UID** to use it.
...
When fetching secrets using Secrets Manager, the system responds that the
...
Individual records can be shared with a Secrets Manager Application, or a
shared folder can be shared with a Secrets Manager Application, which will
give access to all records in that folder.
...
Keeper Secrets Manager supports typed records (V3) only. If a legacy, non-
typed record is queried, Secrets Manager will respond that the record cannot
be found.
...
If you have existing non typed records that you would like to use with Secrets
Manager, we recommend creating a "Login" type record and copying the
information to it.
...
Untyped records can be converted to typed records that Secrets Manager can
utilize by using the `convert` command in Keeper Commander.
...
The Keeper Secrets Manager API throttles requests based on the Device ID. If
your connection is throttled, you will receive an error, such as a 503
response code (the actual message depends on the integration/SDK being used).

###

Details of the rate limits:

The Secrets Manager APIs are set to limit any system that makes more than 200
calls without a 10-second break.

This count resets when the 10 second timer expires, however it renews with
each subsequent request sent in less than 10 seconds. So without a 10 second
pause, the system will continue to be throttled.
