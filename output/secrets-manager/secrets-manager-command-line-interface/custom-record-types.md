...
The Keeper Secrets Manager CLI has a list of default record types that are
...
`keeper.ini` file. The `record_type` doesn't need to be in the same directory
as the `keeper.ini`, it just need to be in locations where the CLI will check
for the `keeper.ini`.
