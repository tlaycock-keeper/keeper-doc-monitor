...
line-interface/config-command?fallback=true)[Keeper Connection
Manager](https://docs.keeper.io/en/keeper-connection-manager/secrets-
manager/secrets-manager-command-line-interface/config-
command?fallback=true)[User Guides](https://docs.keeper.io/en/user-
guides/secrets-manager/secrets-manager-command-line-interface/config-
command?fallback=true)[Release Notes](https://docs.keeper.io/en/release-
notes/secrets-manager/secrets-manager-command-line-interface/config-
command?fallback=true)[Enterprise Guide](https://docs.keeper.io/en/enterprise-
guide/secrets-manager/secrets-manager-command-line-interface/config-
...
connect-cloud/secrets-manager/secrets-manager-command-line-interface/config-
command?fallback=true)[KeeperPAM and Secrets
Manager](https://docs.keeper.io/en/keeperpam/secrets-manager/secrets-manager-
command-line-interface/config-command?fallback=true)[SSO Connect On-
Prem](https://docs.keeper.io/en/sso-connect-on-prem/secrets-manager/secrets-
manager-command-line-interface/config-command?fallback=true)[Keeper
Bridge](https://docs.keeper.io/en/keeper-bridge/secrets-manager/secrets-
manager-command-line-interface/config-command?fallback=true)
...
The record cache creates a copy of your records and will be used if the CLI
cannot access the Secrets Manager server. A copy of records are stored
...
Set the record type schema directory containing custom record type schema
files.
...
`ksm config editor [--app APPLICATION] [--blocking] [--process-name PROCESS
NAME]`
...
`--blocking` Enable to prevent the CLI from prematurely processing your record
data.

`--process-name` Used in blocking to find the real process name in the task
list.
...
On macOS, blocking to wait until the application fully exits. This means the
editor is not running at all, not that the editor windows is closed.

On Windows, blocking will monitor the task lists until the process ends.
However if the application uses a .bat or .cmd file, it will launch the editor
under different name. If this is the case, the `--process-name` needs to be
set. The CLI will monitor that name instead of the name used to launch the
editor.
