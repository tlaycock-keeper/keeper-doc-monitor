...
[Export as PDF](/en/keeperpam/~gitbook/pdf?page=-Mf8fFU-
rv06jD3Us5A_&only=yes&limit=100)
...
Execute arbitrary system calls and replaces templated environment variables
with Keeper vault secrets

##

`exec` command

To perform magic environment variable substitution, use the `**ksm exec**`****
command.
...
Environment variables must be templated so that the CLI can find and replace
...
Below is a Linux bash script example. Before an application is started or a
command is run, any environment variables that start with `**keeper://**` will
be replaced with secret values from the vault. Make sure to replace XXXX with
the Record UID of the secret.
...
[PreviousSync Command](/en/keeperpam/secrets-manager/secrets-manager-command-
