...
[Docs Home](https://docs.keeper.io/en/secrets-manager/integrations/powershell-
plugin?fallback=true)[Keeper Connection
Manager](https://docs.keeper.io/en/keeper-connection-manager/secrets-
manager/integrations/powershell-plugin?fallback=true)[User
Guides](https://docs.keeper.io/en/user-guides/secrets-
manager/integrations/powershell-plugin?fallback=true)[Release
Notes](https://docs.keeper.io/en/release-notes/secrets-
manager/integrations/powershell-plugin?fallback=true)[Enterprise
Guide](https://docs.keeper.io/en/enterprise-guide/secrets-
manager/integrations/powershell-plugin?fallback=true)[MSP
Guide](https://docs.keeper.io/en/msp-guide/secrets-
manager/integrations/powershell-plugin?fallback=true)[SSO Connect
Cloud](https://docs.keeper.io/en/sso-connect-cloud/secrets-
manager/integrations/powershell-plugin?fallback=true)[KeeperPAM and Secrets
Manager](https://docs.keeper.io/en/keeperpam/secrets-
manager/integrations/powershell-plugin?fallback=true)[SSO Connect On-
Prem](https://docs.keeper.io/en/sso-connect-on-prem/secrets-
manager/integrations/powershell-plugin?fallback=true)[Keeper
Bridge](https://docs.keeper.io/en/keeper-bridge/secrets-
manager/integrations/powershell-plugin?fallback=true)
...
[Export as
PDF](/en/keeperpam/~gitbook/pdf?page=-MksKcXNM5-tkgxSqVIb&only=yes&limit=100)
...
[PreviousOracle Key Vault](/en/keeperpam/secrets-manager/integrations/aws-
kms-1)[NextServiceNow](/en/keeperpam/secrets-manager/integrations/servicenow)
...
This page documents the Secrets Manager PowerShell integration. In order to
utilize this integration, you will need:

Secrets Manager requires **PowerShell version 6 or greater**. Microsoft
...
The Keeper Secrets Manager PowerShell plugin utilizes Microsoft PowerShell's
...
Keeper Secrets Manager uses the Microsoft.PowerShell.SecretManagement module
to manage secrets in PowerShell.
...
Install the Keeper Secrets Manager PowerShell extension from the PowerShell
Gallery.
...
If you already have a local secrets extension that you would like to use, you
can skip this step
...
If you already have a local secrets vault registered that you would like to
use, you can skip this step

Register a secret vault for the previously installed secret management
...
`LocalStore` in this example.

The Secret Management extension that you use for local storage may ask you to
create a password for securely accessing the local vault.

Depending on your system settings, you may need to allow PowerShell to trust
external modules. To do this, run the command:
...
Register the Keeper Secrets Manager Vault using the local vault registered
above to save your credentials, and a one time token to connect to Keeper.
...
Alternatively you can use already generated config - replace 'XXX' below with
a base64 encoded config.
...
Set the Keeper vault you just added as the default secret storage. This will
tell the PowerShell SecretsManagement module to use your Keeper vault when
getting and setting secrets.
...
This step is optional, but if you choose not to do it, you may receive secrets
from your default vault if they have the same name, and you will need to add
`-Vault <keeper vault name>` (e.g. `-Vault keeper` ) to `Get-Secret, Set-
Secret` commands
...
title, and escape both `.` and `\` (_ex._ `\.`, `\\`)
...
The secrets shown are any records shared with the Secrets Manager Application.
The Name column displays each record's UID and title.
...
`-AsPlainText` Shows the actual values of the secrets. Otherwise PowerShell
shows them as a `SecureString`
