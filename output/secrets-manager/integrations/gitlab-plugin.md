...
[Docs Home](https://docs.keeper.io/en/secrets-manager/integrations/gitlab-
plugin?fallback=true)[Keeper Connection
Manager](https://docs.keeper.io/en/keeper-connection-manager/secrets-
manager/integrations/gitlab-plugin?fallback=true)[User
Guides](https://docs.keeper.io/en/user-guides/secrets-
manager/integrations/gitlab-plugin?fallback=true)[Release
Notes](https://docs.keeper.io/en/release-notes/secrets-
manager/integrations/gitlab-plugin?fallback=true)[Enterprise
Guide](https://docs.keeper.io/en/enterprise-guide/secrets-
manager/integrations/gitlab-plugin?fallback=true)[MSP
Guide](https://docs.keeper.io/en/msp-guide/secrets-
manager/integrations/gitlab-plugin?fallback=true)[SSO Connect
Cloud](https://docs.keeper.io/en/sso-connect-cloud/secrets-
manager/integrations/gitlab-plugin?fallback=true)[KeeperPAM and Secrets
Manager](https://docs.keeper.io/en/keeperpam/secrets-
manager/integrations/gitlab-plugin?fallback=true)[SSO Connect On-
Prem](https://docs.keeper.io/en/sso-connect-on-prem/secrets-
manager/integrations/gitlab-plugin?fallback=true)[Keeper
...
actions)[NextHashicorp Vault](/en/keeperpam/secrets-
...
This page documents the Secrets Manager GitLab integration. In order to
...
desired destination of the GitLab Pipeline such as an environment variable or
file.
...
A keeper [Secrets Manager configuration](/en/keeperpam/secrets-
manager/about/secrets-manager-configuration) is required to utilize the GitLab
integration.

**1)** Create a Keeper Secrets Manager Configuration. See the
[documentation](/en/keeperpam/secrets-manager/about/secrets-manager-
configuration) for details. The GitLab integration supports **Base64** and
**JSON** configurations.
...
In order to use Keeper Secrets Manager with GitLab, first we need to install
...
This utilizes the [KSM CLI tool](/en/keeperpam/secrets-manager/secrets-
manager-command-line-interface) to get secrets using [Keeper
...
Use `- export <VARIABLE NAME>=$(ksm secret notation <KEEPER NOTATION>)` to set
a secret to an environment variable
...
Use `- ksm secret download -u <UID> --name <SECRET FILENAME> --file-output
"<OUTPUT FILENAME>"` to get a file from the Keeper Vault and save it as a file
to your GitLab Pipeline job.

**Example:**

The following job gets a file named "mykey.pub" that is attached to a Keeper
