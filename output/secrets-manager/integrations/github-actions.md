...
[Docs Home](https://docs.keeper.io/en/secrets-manager/integrations/github-
actions?fallback=true)[Keeper Connection
Manager](https://docs.keeper.io/en/keeper-connection-manager/secrets-
manager/integrations/github-actions?fallback=true)[User
Guides](https://docs.keeper.io/en/user-guides/secrets-
manager/integrations/github-actions?fallback=true)[Release
Notes](https://docs.keeper.io/en/release-notes/secrets-
manager/integrations/github-actions?fallback=true)[Enterprise
Guide](https://docs.keeper.io/en/enterprise-guide/secrets-
manager/integrations/github-actions?fallback=true)[MSP
Guide](https://docs.keeper.io/en/msp-guide/secrets-
manager/integrations/github-actions?fallback=true)[SSO Connect
Cloud](https://docs.keeper.io/en/sso-connect-cloud/secrets-
manager/integrations/github-actions?fallback=true)[KeeperPAM and Secrets
Manager](https://docs.keeper.io/en/keeperpam/secrets-
manager/integrations/github-actions?fallback=true)[SSO Connect On-
Prem](https://docs.keeper.io/en/sso-connect-on-prem/secrets-
manager/integrations/github-actions?fallback=true)[Keeper
Bridge](https://docs.keeper.io/en/keeper-bridge/secrets-
manager/integrations/github-actions?fallback=true)
...
[Export as
PDF](/en/keeperpam/~gitbook/pdf?page=-Mb8Ev8IW9wrMgWzgelp&only=yes&limit=100)
...
Keeper Secrets Manager integration into GitHub Actions for dynamic secrets
retrieval

[PreviousGit - Sign Commits with SSH](/en/keeperpam/secrets-
...
The below overview video covers basic setup and ends with a basic Github
Actions integration.
...
This action securely retrieves secrets from Keeper and places them to the
desired destination of the GitHub Actions runner such as an environment
variable, output parameters of the step or to the file.
...
We recommend storing the configuration in a Github Actions secret and
accessing it as a variable, as shown in the example above.
...
The `secrets` input is the list of secrets that you need to get from Keeper
and put into either an environment variable, GitHub Action output or a file.
...
This action uses GitHub Action's built-in masking, so all variables will
automatically be masked if printed to the console or to logs. This only
obscures secrets from output logs. If someone has the ability to edit your
workflows, then they are able to read and therefore write secrets to somewhere
else just like normal GitHub Secrets.
