...
[Docs Home](https://docs.keeper.io/en/secrets-manager/integrations/gcp-secret-
manager?fallback=true)[Keeper Connection
Manager](https://docs.keeper.io/en/keeper-connection-manager/secrets-
manager/integrations/gcp-secret-manager?fallback=true)[User
Guides](https://docs.keeper.io/en/user-guides/secrets-
manager/integrations/gcp-secret-manager?fallback=true)[Release
Notes](https://docs.keeper.io/en/release-notes/secrets-
manager/integrations/gcp-secret-manager?fallback=true)[Enterprise
Guide](https://docs.keeper.io/en/enterprise-guide/secrets-
manager/integrations/gcp-secret-manager?fallback=true)[MSP
Guide](https://docs.keeper.io/en/msp-guide/secrets-manager/integrations/gcp-
secret-manager?fallback=true)[SSO Connect
Cloud](https://docs.keeper.io/en/sso-connect-cloud/secrets-
manager/integrations/gcp-secret-manager?fallback=true)[KeeperPAM and Secrets
Manager](https://docs.keeper.io/en/keeperpam/secrets-manager/integrations/gcp-
secret-manager?fallback=true)[SSO Connect On-
Prem](https://docs.keeper.io/en/sso-connect-on-prem/secrets-
manager/integrations/gcp-secret-manager?fallback=true)[Keeper
...
To configure the KSM CLI tool, a profile needs to be created with the Keeper
Secrets Manager One Time Access Token.

The simplest way to do this is to initialize the default profile with the
following command:
...
The KSM CLI needs the credentials for the GCP service account to set secrets.
These credentials are stored in a Keeper record which the CLI tool can access
...
A custom record type can be created with the required fields, which makes it
easy and clean to create a record.

To create a custom record type, go to the "Custom Record Types" tab in the
Keeper Vault and hit "Create Type". Create a new record type with hidden
...
To create a credentials record without creating a new record type, the
required fields can be added as custom fields to a standard record.

Create a new record of any type, then add Custom Fields of the 'Hidden Field'
type for each required GCP field. Click "Edit Label" to change the labels for
...
The KSM CLI `sync` command identifies which values to set using mappings that
are defined on the command call. For each mapping passed to the command, a
value with the given name will be populated with the given value from the
Keeper Vault.
...
`VALUE KEY` is the key name that the value will be assigned in GCP Secret
Manager

`KEEPER NOTATION` is a Keeper notation query of a value from a keeper record
to set to the key

Keeper notation is a query notation used by Keeper Secrets Manager to identify
...
Multiple mappings can be added to a single `sync` command `--map "MySQL_PWD"
"jd3[...]i-fd/field/password" --map "MySQL_Login" "jd3[...]i-fd/field/login"`

Ensure that the records referenced by the Keeper Notation queries are in a
shared folder that is shared with your Secrets Manager application
...
To run the sync, use the KSM CLI `sync` command with the credentials record
and value mapping.
...
Put together the KSM sync command with the GCP type. The format looks like the
following:
...
The sync command supports running a dry-run which will identify all changes
that will be made to your GCP Secret Manager values without actually pushing
...
When ready, run the sync command without the dry-run option. This will push
values from your Keeper Vault to GCP Secret Manager
