...
The KSM CLI is available as a binary application for Windows/Mac/Linux or a
pip3 install for any Python environment.
...
**virtualenv**. If you do not use **virtualenv** , the module and binary will
be install into your **$HOME/.local** directory for Linux or macOS. You may
need to include a **PATH** to the **bin** directory.
...
The CLI is initialized as a client device by passing in the One Time Access
Token in the `ksm profile init` command. After initialization, the CLI can be
...
Keeper Secrets Manager commands are run using the `**ksm**` program from the
command line.
...
Sets the keeper.ini configuration file. If not set the CLI will check the
following directories for the _keeper.ini_ file.
