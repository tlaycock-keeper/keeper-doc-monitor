...
plugin)[NextHeroku](/en/keeperpam/secrets-manager/integrations/heroku)
...
The Keeper Secrets Manager HashiCorp Vault integration allows you to use
secrets from the Keeper Vault as a data store for **HashiCorp Vault**.
...
Unzip the plugin and place it into your HashiCorp Vault plugins directory. In
this example the folder is located at `C:\vault\plugins` (Windows) or
`/etc/vault/vault_plugins` (MacOS/ Linux)
...
will need to be re-enabled each time the HashiCorp Vault is started in dev
mode.
...
You will need a hash of the plugin file to enable it with production HashiCorp
Vault servers. This hash can be generated for the Secrets Manager plugin.

Windows 7 and later comes with a built-in tool called CertUtil that can be
used to generate the SHA256 hash. This example will show how to generate a SHA
...
Now that the HashiCorp Vault plugin is installed, a secure connection to the
Keeper Vault needs to be established so that secret credentials can be
accessed. To create this connection, a Secrets Manager configuration needs to
be created and assigned to the plugin.
