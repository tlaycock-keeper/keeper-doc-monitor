...
including:
  ...
  * Keeper Admin Console ([Learn More ](https://docs.keeper.io/enterprise-guide/event-reporting)about the Event Reporting & Alerts Module): <https://docs.keeper.io/enterprise-guide/event-reporting>
...
Below is the list of events generated from Secrets Manager. "Application"
refers to the Secrets Manager Application which is associated to a record or
folder in the Keeper Vault.
...
User ${username} shared record UID ${secret_uid} with KSM application
${app_uid}
...
User ${username} shared folder UID ${secret_uid} with KSM application
${app_uid}
...
User ${username} removed record UID ${secret_uid} from KSM application
${app_uid}
...
User ${username} removed folder UID ${secret_uid} from KSM application
${app_uid}
...
User ${username} changed share permissions for record UID ${secret_uid} for
KSM application ${app_uid}

When user changes share permissions of record share in the Application

app_folder_share_changed

User ${username} changed share permissions for folder UID ${secret_uid} for
KSM application ${app_uid}
...
Access denied to record UID ${secret_uid} from device with blocked IP address
${device_ip}
